...
class crypt
{
	public static function get_hash($password, $salt = null, $salt_length = 32, $algorithm = RUDE_DEFAULT_HASH_ALGORITHM)
	{
		$return_array = false;

		if ($salt === null)
		{
			$salt = static::get_salt($salt_length);

			$return_array = true;
		}


		$hash = static::hash(static::salt($password, $salt), $algorithm);
	...
	public static function get_salt($length = 32, $alphabet = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ')
	{
		return strings::rand($length, $alphabet);
	}

	private static function hash($string, $algorithm = RUDE_HASH_ALGORITHM_DEFAULT)
	{
	...
	public static function get_hash_algorithms()
	{
		return hash_algos();
	}

	public static function is_algorithm_exist($algorithm)
	{
		return items::contains(static::get_hash_algorithms(), $algorithm);
	}

	public static function is_password_valid($password, $hash, $salt, $algorithm = RUDE_HASH_ALGORITHM_DEFAULT)
	{
		$password_hash = static::hash(static::salt($password, $salt), $algorithm);
