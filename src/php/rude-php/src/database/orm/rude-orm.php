...
class orm
{
	...
	public function create($table)
	{
		foreach ($this->strategies as $index => $strategy)
		{
			...
			if (!$columns)
			{
				exception::warning("There are no columns in the `$table` table... Skiping table");
			}
			...
			# 5. filter primary column from other columns

			$columns_except_primary = [];

			foreach ($columns as $column)
			{
				if ($column->field == $column_primary->field)
				{
					continue;
				}

				$columns_except_primary[] = $column;
			}

			$fields_except_primary = items::select($columns_except_primary, 'field');

			$dictionary->set_columns_except_primary($columns_except_primary);
