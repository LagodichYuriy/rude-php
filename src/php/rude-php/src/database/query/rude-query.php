...
class query
{
	...
	{
		$delimiter_order = static::get_delimiter($template, 'ORDER',  '%ORDER%');


		if (!$this->orders)
		{
			return strings::remove($template, 'ORDER' . $delimiter_order . '%ORDER%');
		}
	...
	protected function _sql_limit($template)
	{
		...
		$template = strings::replace($template, '%OFFSET%', (int) $this->offset);
			 ...
			 if (!$this->limit) { return strings::replace($template, '%LIMIT%', 18446744073709551615); } # 2^64 - 1 (big int)
		else                    { return strings::replace($template, '%LIMIT%', (int) $this->limit);   }
	}
	...
	private function to_datetime($string) { return "'" . date('Y-m-d H:i:s', strtotime($string)) . "'"; }
	private function to_time    ($string) { return "'" . date('H:i:s',       strtotime($string)) . "'"; }
	private function to_date    ($string) { return "'" . date('Y-m-d',       strtotime($string)) . "'"; }
	...
	protected function is_float($field_type)     { return strings::starts_with($field_type, 'float');     }
