...
class strings
{
	 ...
	 *
	 * # UTF-8 example:
	 * $string = strings::rand(8, 'πράδειγμα'); # string(16) "εαγρδρμδ"
	 *
	...
	public static function find_last($string, $substring, $offset = null, $case_sensitive = true)
	{
		if ($case_sensitive)
		{
			if (static::is_utf8($string))
			{
				return mb_strrpos($string, $substring, $offset, RUDE_STRING_ENCODING);
			}

			return strrpos($string, $substring, $offset);
		}

		if (static::is_utf8($string))
		{
			return mb_strripos($string, $substring, $offset, RUDE_STRING_ENCODING);
		}

		return strripos($string, $substring, $offset);
	}
	...
	public static function trim($string, $chars = [" ", "\t", "\n", "\r", "\0", "\x0B"])
	{
		if (is_string($chars))
		{
			$chars = strings::chars($chars);
		}

		$string = static::trim_left($string, $chars);
		$string = static::trim_right($string, $chars);
	 ...
	 * @return string
	 */
	public static function trim_left($string, $chars = [" ", "\t", "\n", "\r", "\0", "\x0B"])
	{
		if (is_string($chars))
		{
			$chars = strings::chars($chars);
		...
		{
			while (strings::starts_with($string, $char))
			{
	 ...
	 * @return string
	 */
	public static function trim_right($string, $chars = [" ", "\t", "\n", "\r", "\0", "\x0B"])
	{
		if (is_string($chars))
		{
			$chars = strings::chars($chars);
		...
		foreach ($chars as $char)
		{
			while (strings::ends_with($string, $char))
			{
				$string = char::remove_last($string);
			}
		}
	...
	public static function contains($string, $substring, $case_sensitive = true)
	{
		...
		return static::find($string, $substring, null, $case_sensitive) !== false;
	}
	...
	public static function starts_with($string, $substring, $case_sensitive = true)
	{
		...
		{
			$string    = strings::to_lowercase($string);
			$substring = strings::to_lowercase($substring);
		}
